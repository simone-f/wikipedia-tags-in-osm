...
### Download/update OSM data ###########################################
    ...
    def filter_wikipedia_data_in_OSM_file():
        ...
        command = 'osmfilter %s --keep="wikipedia*=*" --keep-tags="all wikipedia*=*" --drop-version --ignore-dependencies -o=%s' % (app.countryO5M, app.wOSMFile)
        call(command, shell=True)
